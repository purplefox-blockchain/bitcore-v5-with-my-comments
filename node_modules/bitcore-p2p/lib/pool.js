...
Pool.prototype._addPeerEventHandlers = function(peer, addr) {
  ...
  //if a peer is ready, this pool will 'announce' a peerready event to pool's subscribers
  //in p2p module, line 500+, self._pool.on('peerready', p2p-do-something-on-this-event)
  //p2p module consume this peerready event,
  peer.on('ready', function peerReady() {
    self.emit('peerready', peer, addr);
  });
  //for all events defined in Pool.PeerEvents list, we prefix a 'peer' and emit the same event
  //p2p module, index.js, line 500+ has many code like self._pool.on('peerblock',....)
  //all those 'peerxxx' events are emitted here in the code chunk below
  Pool.PeerEvents.forEach(function addPeerEvents(event) {
    //if i receive a 'block', i emit a 'peerblock'
    //if i receive a 'headers', i emit a 'peerheaders'
    //if i recieve a 'inv', i emit a 'peerinv'
    //if i receive a 'getdata', i emit a 'peergetdata' etc etc
