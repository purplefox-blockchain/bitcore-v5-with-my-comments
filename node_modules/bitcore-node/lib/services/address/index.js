...
var AddressService = function(options) {
  ...
  //TODO-low: why do we need to do this? why change regtest to testnet?
...
//from latest to earliest, we query mempool and then query db for txid given an address
...
//remove all tx in the given block
//seems it will only be called by onReorg?
...
//given a block, and one tx in that block,
//remove all inputs and outputs from the db
AddressService.prototype._removeTx = function(tx, block, callback) {

  var self = this;
  var operations = [];

  //for input and outputs, we often process them in parallel
  async.parallelLimit([
...
//if an input is to be removed from the db, we need to put back (i.e add) the corresponding output to the utxo
...
//if an output is to be removed from the db, we need to delete the addr index data, and remove it from the utxo
//note that in _removeInput we didn't delete the addr index data, so we only store output data in db?
...
AddressService.prototype.onBlock = function(block, callback) {
  ...
  log.info('address service .onBlock returning: '+util.inspect(testrunNewOps));
  log.info('the address returned ops count is '+testrunNewOps.length);
  callback(null, operations);
};

//for each input of a tx, save address key, delete from utxo
...
//for each output of a tx, save address key, save utxo
...
//for each tx, process outputs and inputs
